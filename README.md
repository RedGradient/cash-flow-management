...
Очистка Docker после просмотра проекта — удаляет контейнеры `cashflow-app` и `cashflow-postgres`, volume с данными БД и локальный образ приложения:

```bash
make clean
```

Образ `postgres:16-alpine` можно удалить вручную, если он не нужен другим проектам:
